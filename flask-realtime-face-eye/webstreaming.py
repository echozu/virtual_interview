...
def detect_motion(frameCount):
	global vs, outputFrame, lock, onscreen_count, frame_count, percent_list, start_time
	...
	while True:
		...
		onscreen_this_frame = 0
		
		# 如果没有检测到人脸，设置为30%注意力
		if len(faces) == 0:
			onscreen_this_frame = 0.3
			cv2.putText(frame, "NO FACE DETECTED", (50,100), font, 2, (0,0,255), 3)
		else:
			for(x,y,w,h) in faces:
				cv2.rectangle(frame, (x,y), (x+w,y+h), (0,255,0), 2)
				cv2.putText(frame, "FACE", (x+5,y-5), font, 1, (255,255,255), 2)
			...
			for face in eye_faces: #for eye tracking
				...
				# onscreen条件放宽
				if 0.8 < gaze_ratio_avg < 2.2:
					onscreen_this_frame = 1
					cv2.putText(frame,"ON-SCREEN",(50,100),font,2,(0,0,255),3)
				else:
					onscreen_this_frame = 0
					cv2.putText(frame,"OUTSIDE",(50,100),font,2,(0,0,255),3)
		
		onscreen_count += onscreen_this_frame
		frame_count += 1
		# 每秒统计一次
		if time.time() - start_time >= 1.0:
			percent = onscreen_count / frame_count if frame_count > 0 else 0
			percent_list.append(percent)
			with open("log.txt", "a") as f:
				f.write(f"{percent:.3f}\n")
			onscreen_count = 0
			frame_count = 0
			start_time = time.time()
		with lock:
...
log.close()
vs.stop()
atexit.register(plot_attention)